...
if __name__ == "__main__":
    ...
    class DataLoader:
        def __init__(self, data_dir: str, block_size: int = 1024, filemode: str = "r", batch_size=12) -> None:
            ...
            train_ids = tokenizer.encode_ordinary(data)
    ...
    trains = {
        'att' : train(layers=['Attention', 'MLP']*6, attn_cache_kv=True, mlp_gate=True),
        # 'att' : train(layers=['Attention', 'MLP']*6, attn_cache_kv=False),
        # 'catt' : train(layers=['Attention', 'MLP']*6, attn_cache_kv=True),
        # 'attg' : train(layers=['Attention', 'MLP']*6, attn_cache_kv=False, mlp_gate=True),
        # 'cattg' : train(layers=['Attention', 'MLP']*6, attn_cache_kv=True, mlp_gate=True)
    }
